...
#define ARG_DEF_VAL_NOT_SET ""

/* <key, <is_mandatory, default value (if val = ARG_DEF_VAL_NOT_SET no deafult value)>> */
...
class ArgsParser {
    ...
    arguments_t parse_params(int argc, char *argv[]) {
        ...
        for (int i = 0; i < argc; i += 2) {
            ...
            if (!ret.second)
                throw "same key '" + key + "' given more then one";
        }
        ...
        for (auto it = rules.begin(); it != rules.end(); it++) {
            if (it->second.first) {
                ...
                if (params.find(it->first) == params.end() && it->second.second != ARG_DEF_VAL_NOT_SET)
